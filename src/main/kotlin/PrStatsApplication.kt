...
class PrStatsApplication : KoinComponent {
    ...
    suspend fun generatePrStats(prNumber: Int) {
        ...
        Log.i(resources.string("pr_stats_building_for_pr", prNumber))
            ...
            measureTimeMillis {
                ...
                when (statsResult) {
                    ...
                    is PullRequestStatsRepo.StatsResult.Failure -> {
                        Log.w(resources.string("pr_stats_failed_to_generate", prNumber, statsResult.errorInfo.errorMessage))
                    }
                }
            }

        Log.d(resources.string("pr_stats_generation_time", prNumber, authorReportBuildTime.milliseconds))
