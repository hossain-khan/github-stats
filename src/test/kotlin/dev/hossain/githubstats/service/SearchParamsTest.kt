...
/**
 * Contains unit tests for [SearchParams].
 */
class SearchParamsTest {
    ...
    fun `toQuery ensures date range is correctly formatted`() {
        ...
        assertTrue(query.contains(expectedDatePart), "Query should contain encoded date range '$expectedDatePart', but was '$query'")
    }
    ...
    fun `toQuery ensures all parts are URL encoded`() {
        ...
        assertTrue(actualQuery.contains(expectedRepo), "Query missing or incorrect repo encoding. Expected to contain: $expectedRepo")
        assertTrue(actualQuery.contains(expectedAuthor), "Query missing or incorrect author encoding. Expected to contain: $expectedAuthor")
        assertTrue(
            actualQuery.contains(expectedReviewer),
            "Query missing or incorrect reviewer encoding. Expected to contain: $expectedReviewer",
        )
        assertTrue(actualQuery.contains(expectedCreated), "Query missing or incorrect date encoding. Expected to contain: $expectedCreated")
    }
}
