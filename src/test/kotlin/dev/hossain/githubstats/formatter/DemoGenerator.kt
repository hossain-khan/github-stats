...
fun createAuthorCard(authorId: String, index: Int): String {
    ...
    val barChartHtml = BootstrapTemplate.chartJsBarChart(
...
fun createReviewerCard(reviewerId: String, index: Int): String {
    // Sample authors reviewed data
    val authorLabels = listOf("swankjesse", "yschimke", "other-authors")
    val authorData = when(reviewerId) {
        "JakeWharton" -> listOf(1, 1, 1)
        "oldergod" -> listOf(1, 0, 2)
        else -> listOf(2, 1, 1)
    }
    
    val authorBarChartHtml = BootstrapTemplate.chartJsBarChart(
    ...
    val timesBarChartHtml = BootstrapTemplate.chartJsBarChart(
